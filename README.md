...
This package is a compatibility fork based on xreef/PCF8575_library with a fix for INPUT pin reads and active-low buttons.

## Included fix

- Fixed `readBuffer()` so input state is refreshed when either INPUT or INPUT_PULLUP conditions match.
- Fixed `digitalRead()` so it returns the stable current pin level instead of clearing the buffered state after every read.

This is an unofficial fork prepared from the upstream sources.
